...
The watchdog treats completed LLM responses and persisted file checkpoints as activity, while the MCP server owns a fixed 30-minute review deadline. Activity can reset an idle timer; process liveness alone cannot disable the safety limit.
...
The idle limit is derived as `max(15 minutes, OCR_LLM_TIMEOUT + 5 minutes)` and resets on meaningful progress events. The 30-minute review deadline remains the outer safety fuse; neither limit is used to declare normal completion.

The bundled MCP server sets Codex's `tool_timeout_sec` to 31 minutes, leaving one minute for the OCR process to stop and return its terminal timeout error after the 30-minute review deadline. The host timeout is a transport safety limit, not the review completion signal.
...
The MCP surface exposes only `ocr_review`. User cancellation is a host-side interruption propagated through the review context; health checks and preview remain CLI workflows.
